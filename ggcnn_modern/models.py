...
class GGCNN(nn.Module):
    """
    GG-CNN (RSS 2018). Архитектура совпадает с тем, что печатается в README dougsm/ggcnn:
      Conv2d(1->32, k9, s3, p3)
      Conv2d(32->16, k5, s2, p2)
      Conv2d(16->8,  k3, s2, p1)
      ConvT2d(8->8,  k3, s2, p1, outpad1)
      ConvT2d(8->16, k5, s2, p2, outpad1)
      ConvT2d(16->32,k9, s3, p3, outpad1)
      4 головы (pos, cos, sin, width): Conv2d(32->1, k2)
    Выходы: pos in [0,1], cos/sin in [-1,1] (cos/sin(2θ)), width in [0,1] (нормированная).
    """
...
class GGCNN2(nn.Module):
    """
    Более "гладкий" вариант (часто называют GG-CNN2) — совместим по выходам с GGCNN.
    Это компактная fully-conv сетка, удобная для тренировки на больших датасетах (например Jacquard).
    """
